...
const services = [
    {
        ...
        description: "신선한 베이커리와 신선한 생과일주스가 어우러진 완벽한 모닝 세트.",
        image: roomservice1,
    },
    {
        id: 2,
    ...
    },
    {
        ...
        description: "트러플 오일을 더한 프리미엄 프라이드 치킨과 크래프트 맥주의 페어링.",
        image: roomservice4,
    },
    {
        id: 5,
        title: "Whiskey Selection",
        description: "엄선된 싱글 몰트 위스키와 고급 안주가 함께하는 프라이빗 테이스팅.",
        image: roomservice5,
    },
    {
        id: 6,
        ...
        <div className="max-w-7xl mx-auto p-4 pb-32">
            ...
            <div className="mt-20 grid grid-cols-1 md:grid-cols-2 gap-6">
                {services.map((roomservice) => (
                    <div key={roomservice.id} className="bg-white shadow-lg rounded-lg overflow-hidden">
                        <div className="relative">
                            <img src={roomservice.image} alt={roomservice.title} className="w-full h-64 object-cover" />
                        </div>
                        <div className="p-4">
                            <h3 className="text-lg font-semibold">{roomservice.title}</h3>
                            <p className="text-gray-600 text-sm mt-2">{roomservice.description}</p>
                            <div className="mt-4 flex justify-end">
                                <button
                                    className="text-blue-600 text-sm font-semibold hover:underline"
                                    onClick={() => navigate(`/roomservice/${roomservice.id}`, { state: { roomservice } })}
                                >
                                    주문하기 &gt;
                                </button>
                            </div>
